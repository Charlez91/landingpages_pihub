...
var appId = 'F9D84E39-C855-4227-BB82-6C54A2B8FB31';
...
function messageList(message) {
  ...
  if (message.isAdminMessage()) {
    ...
    if (isCurrentUser(user.userId)) {
      ...
      if(user.nickname == GetNickName){
          var msg = '' +
            '<div class="chat-canvas__list_Right"><div class="chat-canvas__list_rightCon">' +
         ...
         var msg = '' +
           '<div class="chat-canvas__list"><div class="chat-canvas__list_leftCon">' +
            '   <div class="chat-canvas__list-name" data-userid="%userid%" data-nickname="%nickname%">' +
            xssEscape(user.nickname) +
            '  </div>' +
            '<label class="chat-canvas__list-separator"></label>' +
            '  <label class="chat-canvas__list-textLef" data-messageid="%messageid%">' +
            convertLinkMessage(xssEscape(message.message)) +
            '  </label>' +
            '</div></div>';
          msgList += msg.replace('%userid%', user.userId).replace('%nickname%', xssEscape(user.nickname)).replace('%messageid%', message.messageId);
